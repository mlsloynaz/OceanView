...
import { Lab1Pane } from "@/features/admin/lab/Lab1Pane";
import { ResearchStatsPane } from "@/features/admin/research-stats/ResearchStatsPane";
import { LearningOutcomesPane } from "./outcomes/LearningOutcomesPane";
import {
  hashForResearchHubView,
  RESEARCH_HUB,
  RESEARCH_HUB_ORDER,
  researchHubViewFromHash,
  type ResearchHubView,
} from "./research-hub";
...
export function ResearchHub() {
  const [view, setView] = useState<ResearchHubView>(() =>
    researchHubViewFromHash(window.location.hash),
  );

  useEffect(() => {
    const sync = () => setView(researchHubViewFromHash(window.location.hash));
    window.addEventListener("hashchange", sync);
    return () => window.removeEventListener("hashchange", sync);
  }, []);

  const openView = useCallback((next: ResearchHubView) => {
    setView(next);
    window.history.replaceState(null, "", `${window.location.pathname}${hashForResearchHubView(next)}`);
  }, []);

  if (view === "outcomes") {
    return <LearningOutcomesPane onBack={() => openView("hub")} />;
  }
  if (view === "research") {
    return <ResearchStatsPane onBack={() => openView("hub")} backLabel="Research" />;
  }
  if (view === "lab1") {
    return <Lab1Pane onBack={() => openView("hub")} backLabel="Research" />;
  }

  return (
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-xl font-semibold text-ocean-foam">Studies</h2>
        <p className="mt-1 max-w-2xl text-sm text-ocean-sand">
          Outcomes labels live setups; Research-Stats and Lab tools stay available for historical
          studies.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {RESEARCH_HUB_ORDER.map((id) => {
          const meta = RESEARCH_HUB[id];
          return (
            <AdminPaneThumbnail
              key={id}
              title={meta.title}
