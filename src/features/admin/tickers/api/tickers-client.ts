...
const MOCK_PROFILES: Record<string, MovementProfile> = {
  AAPL: {
    ...
    fallProb: { "5": 0.12, "8": 0.04, "10": 0.02, "12": 0.01, "15": 0 },
    maDistance: {
      ma20DistancePct: 0.9,
  ...
  if (USE_MOCK) {
    ...
    const ranked = mockCatalog.map((row, index) => ({
      ...
      metrics: {
        ...
        p75MaeDollars: Number(((100 + index * 12) * 0.0065).toFixed(2)),
        p90MaeDollars: Number(((100 + index * 12) * 0.009).toFixed(2)),
